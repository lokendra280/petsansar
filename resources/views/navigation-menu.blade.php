  ...
  <div class="relative pt-6 px-4 sm:px-6 lg:px-8">
    <nav class="relative flex items-center justify-between sm:h-10 lg:justify-start" aria-label="Global">
      ...
      <div class="hidden md:block md:ml-10 md:pr-4 md:space-x-8">
        ...
        <a href="{{route('shiping.add')}}" class="font-medium text-gray-500 hover:text-gray-900">Donation Dog</a>
        <a href="{{route('history.show')}}" class="font-medium text-gray-500 hover:text-gray-900">List of History</a>
        <form method="POST" action="{{ route('logout') }}" x-data>
            @csrf
            <x-jet-dropdown-link href="{{ route('logout') }}"
                     @click.prevent="$root.submit();" class="ml-96">
                {{ __('Log Out') }}
            </x-jet-dropdown-link>
        </form>
      </div>
